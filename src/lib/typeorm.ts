...
export const getDataSource = async (): Promise<DataSource> => {
  if (!dataSource) {
    try {
      ...
      winston.error(
        "Failed to connect to the database. Please make sure PostgreSQL is running."
      );
      winston.error(
        `Connection details: ${process.env.DB_HOST}:${process.env.DB_PORT}`
      );
      ...
      if (process.env.NODE_ENV !== "production") {
        winston.info(
          "For local development, you can start PostgreSQL with Docker:"
        );
        winston.info(
